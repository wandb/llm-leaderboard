...
def test_verify_release_evidence_bundle_rejects_operator_renderer_missing_blocked_status_contract(tmp_path):
    bundle = build_bundle_with_operator_command_script(tmp_path)
    manifest_path = bundle / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    record = next(
    ...
    renderer_script.write_text(
        renderer_script.read_text(encoding="utf-8").replace(
            ...
            "REMOVED_CONTENT_CANARY_BLOCKED_NEXT_ACTIONS",
        ),
        encoding="utf-8",
    )
    refresh_manifest_record_hash(bundle, record["bundle_path"])
...
def test_verify_release_evidence_bundle_rejects_operator_renderer_missing_weave_canary_config_path_policy(
    tmp_path,
):
    bundle = build_bundle_with_operator_command_script(tmp_path)
    manifest_path = bundle / "manifest.json"
    ...
    renderer_script.write_text(
        renderer_script.read_text(encoding="utf-8").replace(
            ...
            'NEMOCLAW_OPENCLAW_CONFIG_PATH = "/tmp/openclaw.json"',
        ),
        encoding="utf-8",
    )
    refresh_manifest_record_hash(bundle, record["bundle_path"])
...
def test_verify_release_evidence_bundle_rejects_operator_renderer_missing_approval_results_budget_contract(
    tmp_path,
):
    bundle = build_bundle_with_operator_command_script(tmp_path)
    manifest_path = bundle / "manifest.json"
...
def test_verify_release_evidence_bundle_rejects_weave_sync_script_without_proof_flags(tmp_path):
    bundle = build_bundle_with_operator_command_script(tmp_path)
    manifest = json.loads((bundle / "manifest.json").read_text(encoding="utf-8"))
    sync_record = next(
        record
    ...
    bundled_sync_script.write_text(
        text.replace('"usage_proven"', '"usage_not_proven"'),
        encoding="utf-8",
    )
    refresh_manifest_record_hash(bundle, sync_record["bundle_path"])
    ...
    assert payload["integrity_ok"] is False
    assert any(
        "Weave Agents sync script missing source contract synced usage proof flag"
        in error
        for error in payload["errors"]
    )
...
def test_verify_release_evidence_bundle_rejects_weave_sync_script_without_native_query_source_invocation(
    tmp_path,
):
    bundle = build_bundle_with_operator_command_script(tmp_path)
    manifest = json.loads((bundle / "manifest.json").read_text(encoding="utf-8"))
    ...
    bundled_sync_script = bundle / sync_record["bundle_path"]
    text = bundled_sync_script.read_text(encoding="utf-8")
    assert "def _query_source_issues(" in text
    bundled_sync_script.write_text(
        text.replace(
            "issues.extend(\n        _query_source_issues(",
            "issues.extend(\n        _manual_source_issues(",
        ),
        encoding="utf-8",
    )
    refresh_manifest_record_hash(bundle, sync_record["bundle_path"])
    ...
    assert any(
        "Weave Agents sync script missing source contract "
        "native Weave query source validator invocation" in error
        for error in payload["errors"]
    )
...
def test_verify_release_evidence_bundle_rejects_agentic_runner_missing_nemoclaw_config_default_contract(
    tmp_path,
):
    bundle = build_bundle_with_operator_command_script(tmp_path)
    manifest_path = bundle / "manifest.json"
    ...
    script_path.write_text(
        script_text.replace(
            ...
            "REMOVED_NEMOCLAW_OPENCLAW_CONFIG_PATH",
        ),
        encoding="utf-8",
    )
    refresh_manifest_record_hash(bundle, record["bundle_path"])
    ...
    assert payload["integrity_ok"] is False
    assert (
        "agentic runner script missing source contract NeMoClaw OpenClaw config default: "
        "scripts/tools/run_openclaw_agent_protocol.py: DEFAULT_NEMOCLAW_OPENCLAW_CONFIG_PATH"
    ) in payload["errors"]
...
def test_verify_release_evidence_bundle_rejects_agentic_evaluator_missing_nemoclaw_config_default(
    tmp_path,
):
    bundle = build_bundle_with_operator_command_script(tmp_path)
    manifest_path = bundle / "manifest.json"
    ...
    assert (
        "agentic runner script missing source contract "
        "Agentic Math NeMoClaw OpenClaw config default: "
        "scripts/evaluator/agentic_math.py: DEFAULT_NEMOCLAW_OPENCLAW_CONFIG_PATH"
    ) in payload["errors"]
...
def test_verify_release_evidence_bundle_rejects_full_batch_missing_budget_approval_cap_contract(
    tmp_path,
):
    bundle = build_bundle_with_operator_command_script(tmp_path)
    manifest_path = bundle / "manifest.json"
...
def test_verify_release_evidence_bundle_rejects_weave_agents_verifier_missing_native_api_contract(
    tmp_path,
):
    bundle = build_bundle_with_operator_command_script(tmp_path)
    manifest = json.loads((bundle / "manifest.json").read_text(encoding="utf-8"))
    ...
    script_text = script_path.read_text(encoding="utf-8")
    script_path.write_text(
        script_text.replace('AGENTS_QUERY_ENDPOINT = "/agents/query"', 'AGENTS_QUERY_ENDPOINT = "/local/query"'),
        encoding="utf-8",
    )
    refresh_manifest_record_hash(bundle, record["bundle_path"])
...
def test_verify_release_evidence_bundle_rejects_weave_canary_without_runner_script(
    tmp_path,
):
    bundle = build_bundle_with_operator_weave_content_canary_command(tmp_path)
    manifest_path = bundle / "manifest.json"
...
def test_verify_release_evidence_bundle_rejects_weave_canary_runner_missing_external_approval_block(
    tmp_path,
):
    bundle = build_bundle_with_operator_weave_content_canary_command(tmp_path)
    manifest = json.loads((bundle / "manifest.json").read_text(encoding="utf-8"))
    ...
    script_path.write_text(
        script_text.replace(
            ...
            'if False and not external_action_approval.get("valid"):',
        ),
        encoding="utf-8",
    )
    ...
    assert payload["integrity_ok"] is False
    assert (
        "agentic runner script missing source contract external action "
        "pre-OpenClaw block: scripts/tools/run_weave_agents_content_canary.py: "
        'if not external_action_approval.get("valid"):'
    ) in payload["errors"]
