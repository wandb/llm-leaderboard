...
最新の機械判定は `temp/taiwan_release_gate_20260701T130612Z.json` です。状態は `not_ready`、bundle整合性はOK、checked filesは655、verification errorsは0です。つまり「証跡bundleそのものは壊れていないが、本番公開条件はまだ満たしていない」という状態です。現在の残ブロッカーは `weave_content_canary`、`wandb_completion`、`paid_run_review_package`、`one_model_full_canary` の4つです。
...
NeMoClawについては、ローカルruntime/sandbox/native weave readinessはrelease gate上で通っており、`nemoclaw_readiness` は現在のブロッカーから外れています。Agentic Math/SWE向けには、NeMoClaw config、task-agent実行経路、SWE-Bench Pro copy-mode checkout transfer、post-install/adoption/canary readiness、operator command policy、release bundle verifierまで実装済みです。さらに、OpenClaw session JSONLで「問題提示前のtool実行」や「最終回答後のtool実行」を検出してnon-scoreableにするガードを入れ、そのrunner source自体もrelease evidence bundleに含めてsource-contract検証するようにしました。A600/A601では、Weave content canaryを実行する直前にNeMoClaw sandbox内の `/sandbox/.openclaw/openclaw.json` を確認するpreflight証跡を必須化し、operator/remediation commandにも同じconfig pathを明示しました。A602では、その引数形のprepare-only canary plan/gateをローカル生成し、外部APIなしで証跡形式を確認しています。A603では、同じ `/sandbox/.openclaw/openclaw.json` 明示指定を実ベンチ側のAgentic Math/SWE batch handoffにも拡張し、paid agentic/full batch command、生成config、W&B completion run.config期待値、operator policy、release-bundle verifierで欠落や別pathを拒否するようにしました。A604では、実際にOpenAI-direct canaryのAgentic Math/SWE batchをprepare-onlyで生成し、生成config、execution plan、paid-run reviewのすべてに同じpathが入り、有料APIやW&B writeが発生しないことを確認しています。A605では、prepare-only content-canary gateが残っている状態をprovider/runtime障害のように表示せず、「まだlive canary未実行」としてoperator planに出すようにしました。A606では、NeMoClaw post-install handoff自体にも `/sandbox/.openclaw/openclaw.json` を明示し、adoption doctor、operator docs verifier、release-bundle verifierで欠落を拒否するようにしました。A607では、Agentic Math/SWE runnerのcache keyとOpenClaw protocol sidecar metadataにもOpenClaw config sourceを入れ、sandbox config sourceが変わった後に古いresult/patch/sidecarを再利用しないようにしました。A608では、fresh successや空patch扱いの結果採用時にもsidecarのtask、prompt、model、tool policy、config sourceが現在のcache keyと一致することを必須にしました。A609では、NeMoClaw-scopedなMath/SWE sidecarについて、required session auditがmissingまたはfailedなら点数やpatchとして採用しないrunner-side guardも追加しました。A610では、既存の `result.json` や `patch_record.json` をcache reuseする場合にも同じrequired session audit成功を要求し、古いcacheが本番runに紛れ込む経路を塞ぎました。A611では、cache側にもconversation-order失敗の拒否を追加し、session順序が壊れた古いcacheが再利用されないようにしました。A612では、tool policy違反を含む既存cacheの再利用も拒否します。A613では、Weave sidecar loggingが明示失敗した既存cacheの再利用も拒否します。A614では、release bundleのsource contractがこれらcache guardの関数定義だけでなく、Math/SWEのcache採用経路から実際に呼ばれていることも検証します。A615では、同じsource contractでNeMoClaw session-audit cache guardの呼び出しも必須にしました。A616では、新規sidecar採用直前のidentity guardとsession-audit guardの呼び出しもrelease bundleで検証します。A617では、Agentic Mathの既存成功sidecarをstrict Weave relogした後にもsidecar_matches_cacheを再実行し、relog後にtask、prompt、model、tool policy、config source、session auditのいずれかが現在条件とずれたsidecarを採用しないようにしました。A618では、既存結果をW&Bへrelogする回収経路でも、明示的なconversation-order失敗、tool-policy失敗、Weave sidecar失敗を含むAgentic Math/SWE行を正式化しないようにしました。A619では、audit_taiwan_existing_results.py自体もrelease bundleのsource-contract対象に加え、既存結果audit側のMath/SWE observability validation callが消えたbundleを拒否するようにしました。A620では、Agentic Math/SWEのW&B relog output tableにtool-policy、conversation-order、Weave-sidecar関連列を安定して残し、その列が落ちたbundleもsource contractで拒否するようにしました。A621では、W&B completion verifier自体もAgentic Math/SWE output tableの観測列をsummaryまたはW&B table JSONから検査し、completion JSONとrelease bundleで列証跡が欠けた場合に拒否するようにしました。A622では、Weave Agents completion proofについても、visible content、visible user input、tool span、tool content、usage、no-error span、request_model、trace order checksを必須化し、構造だけまたはusageなしのAgents traceがpaid-run reviewやrelease bundleを通らないようにしました。A623では、そのproofをpaid-run reviewへ同期する段階でも、message/input/tool/tool-content/usage/no-errorの明示proof flagsが欠けたsync dry-run entryをproduction readinessとrelease bundleで拒否するようにしました。A624では、operator handoffに含まれる同期スクリプト本体もrelease bundle内でsource-contract検証し、proof flagsやusage証跡チェックを落とす退行を検出します。A625では、Weave verifier本体も同じcontent/tool/usage/no-error/timestamp/order/request_model条件でsource-contract検証し、verifierだけ弱くなる退行を検出します。A626では、そのverifierがnative W&B Agents API base URL、/agents/query、/agents/spans/query、HTTP POST、query_source endpoint fieldsを保持していることもbundleで検証し、local/manual JSONやsidecar-only証跡への退行を検出します。A627では、sync_weave_agents_completion_to_paid_review.py側もnative query_source validator、endpoint constants、count fields、synced-entry query_source fieldsを保持していることをbundleで検証し、paid-review同期段階で証跡が弱くなる退行を検出します。A628では、そのvalidatorがpayload validation pathから実際に呼ばれていることもsource-contract検証し、定義だけ残して呼び出しを外す退行を検出します。A629では、operator rendererの外部承認path検証、content-canary gate検証、OpenAI-direct canary batch検証、approval-scope検証も実呼び出しまでsource-contract検証し、shell handoff直前の安全検証が定義だけ残って迂回される退行を検出します。A630では、check_taiwan_nemoclaw_adoption.pyのW&B/Weave runtime policy criterionとruntime network allowlist criterionがadoption criteria listから実際に呼ばれていることもsource-contract検証し、採用判断からruntime policy検証だけ抜ける退行を検出します。A631では、post-install verifierとrelease-bundle verifierがsetup_check、protocol_preflight、canary_readiness、adoption_checkの各handoff commandにsandbox選択tokenを必須化し、default sandboxへ暗黙に戻る退行を検出します。A632では、同じhandoff commandのsandbox/config path値まで一致検査し、別sandboxや別OpenClaw configへ向いた証跡を拒否します。
...
2026-07-01 22:06 JST時点では、NeMoClawはローカル環境でAgentic Math/SWE評価のsandbox backendとして使える状態に到達しています。`nemoclaw_readiness` は最新release gateでpassedです。評価runner側も、Agentic Math task-agent経路、SWE-Bench Pro copy-mode checkout transfer、OpenClaw session JSONLの会話順序ガード、W&B/Weave証跡の検証入口、release bundleへのrunner source同梱まで実装済みです。content canaryについても、実行直前にNeMoClaw sandbox内OpenClaw configを確認するpreflight、operator handoff上の明示config path、release bundle verifierでの欠落拒否まで実装済みです。さらにAgentic Math/SWEの実ベンチbatch handoffでも、同じ sandbox OpenClaw config pathを明示しないpaid agentic/full実行は事前に拒否されます。A604のprepare-only生成では、その設定が実際のOpenAI-direct canary configとreview JSONへ反映されることも確認済みです。A605では、operator planのcontent-canary次アクションが、prepare-only/not_runの証跡をprovider障害と誤読させない文言に変わっています。A606では、post-install verification commandも同じsandbox OpenClaw config pathを明示し、現在ホストのpost-install検証でもそのcommandが出ることを確認済みです。A607では、Agentic Math/SWEのcache再利用条件にもOpenClaw config sourceを入れ、protocol sidecarにも同じsource identityを保存するようにしました。A608では、runnerが新しく生成されたsidecarを採用する直前にも同じidentityを検査するため、古いsidecarや別config由来のsidecarが点数やpatchとして混入しにくくなっています。A609では、NeMoClaw-scopedな新規sidecarや回収sidecarにrequired session auditの成功がない場合も、点数やpatchとして採用しないようにしました。A610では、既存cacheの再利用にも同じaudit成功条件をかけたため、古いローカル結果を誤って正式証跡へ流用する余地がさらに減っています。A611では、conversation_order_ok=false または conversation_order.ok=false を持つcacheも再利用しないため、会話順序違反の既存cacheを正式runに混ぜにくくなりました。A612では、tool_policy_ok=false または tool_policy_violations があるcacheも再利用しないため、禁止tool使用を含む古いcacheを正式runに混ぜにくくなりました。A613では、weave_sidecar.ok=false または weave_sidecar_ok=false が残ったcacheも再利用しないため、Weave記録に明示失敗した古いcacheを正式runに混ぜにくくなりました。A614/A615では、これらのcache guardが定義だけ残って未使用になる退行もrelease bundle verifierで検出します。A616では、新規sidecar採用直前のidentity guardとNeMoClaw session-audit guardも実呼び出し検証の対象です。A617では、Agentic Mathの既存成功sidecarをWeaveへrelogした後にも同じidentity検査をかけるため、relogで書き換わったsidecarがそのまま採点に流れません。A618では、既存結果relogとexisting-results auditもrunner採用条件に寄せ、明示的なconversation-order/tool-policy/Weave失敗を含むローカル結果をW&B正式化候補から外します。A619では、そのexisting-results audit側の観測失敗rejectがsource contractで守られるため、audit経路だけ退行したbundleも検出できます。A620では、W&B relog後のoutput tableでもtool-policy、conversation-order、Weave-sidecar状態を安定列として確認できます。A621では、W&B completion verifierでも同じoutput table列を検査するため、行数だけ揃っていて観測列が落ちたrunはcompletion proofとして通りません。A622では、Weave Agents completion verifierのreview採用条件も強め、content、user input、tool span、tool content、usage、no-error span、request_model、trace orderの各証跡がないtraceを正式証跡として採用しません。A623では、その強いverifier proofがpaid-run reviewへのsync dry-run entryでも明示proof flagsとして保持され、sync時に弱い証跡へ劣化した場合はproduction readinessとrelease bundleが拒否します。A624では、同期スクリプト自体もrelease bundleのsource-contract対象にして、operator handoff経路からproof flagsが消えた場合にbundle integrityを落とします。A625では、verify_taiwan_weave_agents.py本体も同じcontent/tool/usage/no-error/timestamp/order/request_model条件でsource-contract検査されるため、verifier側だけ弱くなる退行もbundle integrityで検出します。A626では、そのverifierがnative W&B Agents API base URL、/agents/query、/agents/spans/query、HTTP POST、query_source endpoint fieldsを保持していることもbundleで検証し、local/manual JSONやsidecar-only証跡への退行を検出します。A627では、sync_weave_agents_completion_to_paid_review.py側もnative query_source validator、endpoint constants、count fields、synced-entry query_source fieldsを保持していることをbundleで検証し、paid-review同期段階で証跡が弱くなる退行を検出します。A628では、そのvalidatorがpayload validation pathから実際に呼ばれていることもsource-contract検証し、定義だけ残して呼び出しを外す退行を検出します。A629では、operator rendererの外部承認path検証、content-canary gate検証、OpenAI-direct canary batch検証、approval-scope検証も実呼び出しまでsource-contract検証し、shell handoff直前の安全検証が定義だけ残って迂回される退行を検出します。A630では、check_taiwan_nemoclaw_adoption.pyのW&B/Weave runtime policy criterionとruntime network allowlist criterionがadoption criteria listから実際に呼ばれていることもsource-contract検証し、採用判断からruntime policy検証だけ抜ける退行を検出します。A631では、post-install verifierとrelease-bundle verifierがsetup_check、protocol_preflight、canary_readiness、adoption_checkの各handoff commandにsandbox選択tokenを必須化し、default sandboxへ暗黙に戻る退行を検出します。A632では、同じhandoff commandのsandbox/config path値まで一致検査し、別sandboxや別OpenClaw configへ向いた証跡を拒否します。ただし、実ベンチ問題をNeMoClaw経由で本番同等に完走し、native Weave trace、W&B completion、paid-run review、official evaluator結果まで揃えた証跡はまだありません。
...
latest gate: temp/taiwan_release_gate_20260701T124440Z.json
latest pointer: temp/latest_taiwan_release_gate.json
latest pointer verification: temp/latest_taiwan_release_gate_verify_20260701T124440Z.json
evidence bundle: outputs/taiwan_release_evidence/bundle_20260701T124440Z
release gate pointer proof: outputs/taiwan_release_evidence/bundle_20260701T124440Z/release_gate_pointer_proof.json
bundle integrity: OK
checked files: 642
...
最新の正式判定ファイルは、常に `temp/latest_taiwan_release_gate.json` から辿れます。2026-07-01 21:44 JST時点の実体は次です。

```text
release gate: temp/taiwan_release_gate_20260701T124440Z.json
latest pointer: temp/latest_taiwan_release_gate.json
latest pointer verification: temp/latest_taiwan_release_gate_verify_20260701T124440Z.json
operator plan: temp/taiwan_release_operator_plan_20260701T124440Z.md
evidence bundle: outputs/taiwan_release_evidence/bundle_20260701T124440Z
bundle verification: temp/taiwan_release_evidence_bundle_verify_20260701T124440Z.json
release gate pointer proof: outputs/taiwan_release_evidence/bundle_20260701T124440Z/release_gate_pointer_proof.json
status: not_ready
checked files: 642
verification errors: 0
...
3. temp/taiwan_release_operator_plan_20260701T124440Z.md
   次に人間が実行・承認すべき操作を見る。

4. outputs/taiwan_release_evidence/bundle_20260701T124440Z/summary.md
...
6. outputs/taiwan_release_evidence/bundle_20260701T124440Z/external_action_approval_packet.md
   有料API、W&B write、NeMoClaw install、scope confirmationの承認要件を見る。

7. outputs/taiwan_release_evidence/bundle_20260701T124440Z/operator_plan.md
...
11. temp/taiwan_existing_results_audit_20260701T124440Z.json
...
13. temp/taiwan_nemoclaw_adoption_check_20260701T124440Z.md
   NeMoClawがAgentic Math/SWE向けにadoptable_for_agentic_benchmarksであることを見る。
   temp/nemoclaw_setup_check_20260701T124440Z.json は runtime/sandbox状態の最新証跡を示す。
...
14. temp/nemoclaw_post_install_verification_20260701T124440Z.json
...
release gate / evidence bundle:
  最新bundleは609ファイルを検証し、integrityはOK。
