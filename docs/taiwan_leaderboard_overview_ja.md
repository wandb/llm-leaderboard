...
最新の機械判定は `temp/taiwan_release_gate_20260701T130100Z.json` です。状態は `not_ready`、bundle整合性はOK、checked filesは653、verification errorsは0です。つまり「証跡bundleそのものは壊れていないが、本番公開条件はまだ満たしていない」という状態です。現在の残ブロッカーは `weave_content_canary`、`wandb_completion`、`paid_run_review_package`、`one_model_full_canary` の4つです。
