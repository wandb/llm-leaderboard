...
最新の機械判定は `temp/taiwan_release_gate_20260701T142938Z.json` です。状態は `not_ready`、bundle整合性はOK、checked filesは666、verification errorsは0です。つまり「証跡bundleそのものは壊れていないが、本番公開条件はまだ満たしていない」という状態です。現在の残ブロッカーは `weave_content_canary`、`wandb_completion`、`paid_run_review_package`、`one_model_full_canary` の4つです。
...
NeMoClawについては、ローカルruntime/sandbox/native weave readinessはrelease gate上で通っており、`nemoclaw_readiness` は現在のブロッカーから外れています。Agentic Math/SWE向けには、NeMoClaw config、task-agent実行経路、SWE-Bench Pro copy-mode checkout transfer、post-install/adoption/canary readiness、operator command policy、release bundle verifierまで実装済みです。さらに、OpenClaw session JSONLで「問題提示前のtool実行」や「最終回答後のtool実行」を検出してnon-scoreableにするガードを入れ、そのrunner source自体もrelease evidence bundleに含めてsource-contract検証するようにしました。A631/A632ではpost-install handoff commandのsandbox選択tokenとsandbox/config値を検査し、A633/A634では同梱された `verify_nemoclaw_post_install.py` 自体のsource-contractと実bundle同梱を必須化し、A635では `check_taiwan_nemoclaw_adoption.py` と `check_taiwan_canary_readiness.py`、A636では `install_nemoclaw.sh`、A637では `verify_weave_agents_content_canary_result.py`、A638では `run_weave_agents_content_canary.py` の実bundle同梱とsource-contractも必須化し、A639では未承認またはNeMoClaw config preflight失敗でOpenClaw前に止まる場合も command_result と gate JSON に paid_api_attempted=false と明示ステータスを残すようにしたため、値検査、adoption/readiness検査、content-canary gate/runner検査、ブロック時の監査証跡を実装から外したbundleは通りません。
...
latest gate: temp/taiwan_release_gate_20260701T142938Z.json
latest pointer: temp/latest_taiwan_release_gate.json
latest pointer verification: temp/latest_taiwan_release_gate_verify_20260701T142938Z.json
evidence bundle: outputs/taiwan_release_evidence/bundle_20260701T142938Z
release gate pointer proof: outputs/taiwan_release_evidence/bundle_20260701T142938Z/release_gate_pointer_proof.json
bundle integrity: OK
checked files: 666
...
最新の正式判定ファイルは、常に `temp/latest_taiwan_release_gate.json` から辿れます。2026-07-01 23:29 JST時点の実体は次です。

```text
release gate: temp/taiwan_release_gate_20260701T142938Z.json
latest pointer: temp/latest_taiwan_release_gate.json
latest pointer verification: temp/latest_taiwan_release_gate_verify_20260701T142938Z.json
operator plan: temp/taiwan_release_operator_plan_20260701T142938Z.md
evidence bundle: outputs/taiwan_release_evidence/bundle_20260701T142938Z
bundle verification: temp/taiwan_release_evidence_bundle_verify_20260701T142938Z.json
release gate pointer proof: outputs/taiwan_release_evidence/bundle_20260701T142938Z/release_gate_pointer_proof.json
status: not_ready
checked files: 666
verification errors: 0
...
3. temp/taiwan_release_operator_plan_20260701T142938Z.md
   次に人間が実行・承認すべき操作を見る。

4. outputs/taiwan_release_evidence/bundle_20260701T142938Z/summary.md
...
6. outputs/taiwan_release_evidence/bundle_20260701T142938Z/external_action_approval_packet.md
   有料API、W&B write、NeMoClaw install、scope confirmationの承認要件を見る。

7. outputs/taiwan_release_evidence/bundle_20260701T142938Z/operator_plan.md
...
11. temp/taiwan_existing_results_audit_20260701T142938Z.json
...
13. temp/taiwan_nemoclaw_adoption_check_20260701T142938Z.md
   NeMoClawがAgentic Math/SWE向けにadoptable_for_agentic_benchmarksであることを見る。
   temp/nemoclaw_setup_check_20260701T142938Z.json は runtime/sandbox状態の最新証跡を示す。
...
14. temp/nemoclaw_post_install_verification_20260701T142938Z.json
