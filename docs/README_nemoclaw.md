...
- Weave Agents completion proof is enforced both at verifier time and sync
  ...
  silently drop this proof enforcement. The bundled
  `verify_taiwan_weave_agents.py` verifier itself is now source-contract checked
  for the same content, tool, usage, no-error, timestamp/order, and
  request_model obligations. A626 also source-contract checks the native W&B
  Agents API base URL, `/agents/query`, `/agents/spans/query`, HTTP POST helper,
  query calls, and query_source endpoint fields so the verifier cannot regress
  to local or sidecar-only proof. A627 applies the same source-contract posture
  ...
  different sandbox or OpenClaw config path is rejected. A633 adds a
  release-bundle source contract for the bundled post-install verifier itself.
  ...
  evidence, and NeMoClaw OpenClaw preflight proof. A638 also requires the formal
  bundle to include `run_weave_agents_content_canary.py` and source-contract
  ...
  status before OpenClaw/provider execution can start. A640 also makes
...
the same audit condition before reuse, and cached patch records with an
explicit failed conversation-order check, tool-policy violation, or failed
Weave sidecar logging result are not reused.
...
same audit condition before reuse, and cached results with an explicit failed
conversation-order check, tool-policy violation, or failed Weave sidecar logging
