...
| Agentic SWE candidate: DeepSWE Budgeted / Essential | Pier task definitions: full 113, fixed `leaderboard_50`, fixed `pilot_16`, and correlation-selected `essential_10`/`16`/`20`/`30`. Taiwan base config defaults to `essential_16`; full configs keep DeepSWE disabled unless explicitly enabled. | `datacurve-ai/deep-swe` local checkout under `external/deep-swe/tasks`; official execution is through Pier. Essential subsets are selected from public DeepSWE v1.1 rollout outcomes. | No translation. Tasks are code/repository repair instructions; repository state is provided by Pier/OpenClaw workspace access, not prompt embedding. Taiwan-specific value is the explicit budgeted condition: 40 tool calls, 40 agent turns, and token caps. | Local manifests under `data/taiwan/deepswe`; no W&B dataset artifact logged yet. Selection method documented in `docs/deepswe_essential_subset_selection.md`. |
| BFCL v4 zh-TW | Default `full` profile: 17 categories and 496 logical cases (`16 x 30` plus all 16 eligible `live_relevance` cases), including the official v4 memory/web Agentic domains. Parallel, long-context, and format-sensitivity categories are excluded; multi-turn cases are at most 3 turns; selection is deterministic with seed `20260724`. A 12-category/346-case `core` profile remains only for provider diagnostics. | Official BFCL v4 source from `ShishirPatil/gorilla`, pinned at `6ea57973c7a6097fd7c5915698c54c17c5b1b6c8`. | 3,244 user-facing fields localized with `gpt-5.4-mini-2026-03-17`; function names, argument names, enums, required fields, literals, and answer structures are preserved. Audit: 0 structural issues and 0 Japanese kana. Web search defaults to a free direct DuckDuckGo backend with bounded retry, pacing, and SQLite cache; SerpAPI remains selectable in YAML. | W&B `bfcl-v4-zh-tw:production` v0, digest `723977c468ce77ec5c746515eada3564`; local source under `data/taiwan/bfcl-v4-zh-tw`. Core OpenAI and Anthropic canaries passed with 0 runtime errors; a fresh 496-case release run remains required. |
