...
if not isinstance(plugin_config, dict):
    raise SystemExit(f"plugins.entries.{plugin_id}.config must be an object in {path}")
plugin_config["liveConfigPath"] = str(path)
...
if [[ -n "${SANDBOX}" ]]; then
  ...
  if nemoclaw sandbox cp --help >/dev/null 2>&1; then
    nemoclaw sandbox exec "${SANDBOX}" --no-tty --timeout 30 -- rm -rf \
      "${REMOTE_ARCHIVE}" "${REMOTE_PATCH}"
  ...
  nemoclaw sandbox exec "${SANDBOX}" --no-tty --timeout 30 -- python3 -c 'import json, pathlib; p=pathlib.Path("/sandbox/.openclaw/openclaw.json"); c=json.loads(p.read_text(encoding="utf-8")); plugins=c.setdefault("plugins", {}); allow=plugins.setdefault("allow", []); pid="nejumi-budget-guard"; allow.append(pid) if isinstance(allow, list) and pid not in allow else None; entries=plugins.setdefault("entries", {}); entry=entries.setdefault(pid, {}); entry["enabled"]=True; hooks=entry.setdefault("hooks", {}); hooks["allowConversationAccess"]=True; hooks.setdefault("timeoutMs", 1000); plugin_config=entry.setdefault("config", {}); plugin_config["liveConfigPath"]=str(p); p.write_text(json.dumps(c, ensure_ascii=False, indent=2)+"\n", encoding="utf-8"); print(f"Patched OpenClaw config for {pid}: {p}")'
