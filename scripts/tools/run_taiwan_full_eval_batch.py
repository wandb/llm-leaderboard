...
def _empty_list(value: object) -> bool:
    return isinstance(value, list) and not value


def weave_content_canary_gate_contract_issues(payload: dict) -> list[str]:
    """Validate that a passed gate came from the native Weave verifier path."""
    issues: list[str] = []
    ...
    if not _empty_list(payload.get("weave_verifier_validation_issues")):
    ...
    if not _empty_list(payload.get("agents_diagnostic_validation_issues")):
        issues.append("agents_diagnostic_validation_issues must be an empty list")
    if not _empty_list(payload.get("failed_checks")):
    ...
    if not isinstance(health, dict):
        ...
        if not _int_at_least(health.get("spans_with_valid_timestamps"), 1):
