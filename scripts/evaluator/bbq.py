...
categories = [
    ...
    "Disability_status",
    "Gender_identity",
    "Gender_identity(names)",
    "Nationality",
    "Physical_appearance",
    "Race_ethnicity",
    "Race_ethnicity(names)",
    "Religion",
    "SES",
    "Sexual_orientation",
]
...
def evaluate_n_shot(few_shots: bool):
    ...
    for task in tasks:
        ...
        for subset in ("test", "dev"):
            ...
            if cfg.testmode:
                ...
                test_max_num_samples = 4
                val_max_num_samples = 4 # use 44 samples for validation (total 440)
    ...
    leaderboard_table = pd.DataFrame([{
        ...
        "avg_bias_score": (score_dict[f"{dataset_name}_{num_few_shots}shot_dev_biasscore_DIS"] + score_dict[f"{dataset_name}_{num_few_shots}shot_dev_biasscore_AMB"]) / 2,
    }])
